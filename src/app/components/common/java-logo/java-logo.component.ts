...
@Component({
  selector: 'app-java-logo',
  imports: [],
  templateUrl: './java-logo.component.html',
  styleUrl: './java-logo.component.css'
})
export class JavaLogoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async ngAfterViewInit() {
    ...
    this.modelBottomOffset = model.position.y+10.2;
  ...
  private handlePointerMove = (event: MouseEvent) => {
    ...
    if (intersects.length > 0 && !this.animationState.isHovered) {
      ...
      this.animationState.bounceVelocity = 1.0;
